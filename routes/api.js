...
// Register agent with starting balance
router.post('/agents/register', (req, res) => {
    ...
    db.run(
        'INSERT INTO agents (agent_id, name, token, balance) VALUES (?, ?, ?, 100)',
        [agent_id, name, token],
        (err) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            res.json({ agent_id, token, balance: 100, message: 'Agent registered successfully' });
        }
    );
});
...
// Stand action with betting
router.post('/tables/:table_id/stand', authMiddleware, (req, res) => {
    const { table_id } = req.params;
    const { hand_value, result, bet_amount } = req.body;
    getAgentIdFromToken(req.token, (agent_id) => {
        if (!agent_id) return res.status(401).json({ error: 'Invalid token' });

        // Validate bet amount
        if (!bet_amount || bet_amount < 5) {
            return res.status(400).json({ error: 'Bet amount must be at least 5 units' });
        }
        ...
        db.get('SELECT current_round_id FROM tables WHERE table_id = ?', [table_id], (err, table) => {
            ...
            db.run(
                'INSERT INTO actions (table_id, round_id, agent_id, action_type, hand_value, bet_amount, result) VALUES (?, ?, ?, ?, ?, ?, ?)',
                [table_id, table.current_round_id, agent_id, 'stand', hand_value, bet_amount, result],
                (err) => {
                    if (err) return res.status(500).json({ error: err.message });

                    // Update agent stats and balance
                    let balanceChange = 0;
                    if (result === 'win') {
                        db.run('UPDATE agents SET wins = wins + 1 WHERE agent_id = ?', [agent_id]);
                        balanceChange = bet_amount; // Win = +bet
                    } else if (result === 'loss') {
                        db.run('UPDATE agents SET losses = losses + 1 WHERE agent_id = ?', [agent_id]);
                        balanceChange = -bet_amount; // Loss = -bet
                    } else if (result === 'bust') {
                        db.run('UPDATE agents SET busts = busts + 1 WHERE agent_id = ?', [agent_id]);
                        balanceChange = -bet_amount; // Bust = -bet
                    }

                    // Update balance
                    db.run('UPDATE agents SET balance = balance + ? WHERE agent_id = ?', [balanceChange, agent_id], (err) => {
                        if (err) return res.status(500).json({ error: err.message });
                        res.json({ result, balanceChange });
                    });
                }
            );
        });
...
router.get('/tables/:table_id/log', (req, res) => {
    ...
    db.all(
        `SELECT a.agent_id, a.action_type, a.hand_value, a.bet_amount, a.result, a.timestamp, ag.name
...
// Leaderboard with balance
router.get('/leaderboard', (req, res) => {
    db.all(
        'SELECT agent_id, name, wins, losses, busts, balance FROM agents ORDER BY balance DESC',
        (err, agents) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json(agents);
        }
